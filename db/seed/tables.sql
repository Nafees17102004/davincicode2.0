...
CREATE TABLE FIELD_SNIPPET_MAP (
    ...
    FOREIGN KEY (LANGUAGE_ID) REFERENCES languages(id) ON DELETE CASCADE,
     -- ensure one snippet per field+language
);
...
CREATE TABLE ADD_FORM_TABLE (
    ...
    C2C_UDATE DATETIME ON UPDATE CURRENT_TIMESTAMP,
    C2C_UUSER VARCHAR(100),
    FIELD_TYPE_ID INT NULL AFTER FIELD_SOURCE_LOV_DET_ID,
    STORING_SP VARCHAR(100) NULL AFTER FIELD_ORDER_LOV_DET_ID,
    FOREIGN KEY (FIELD_TYPE_ID) REFERENCES FIELD_TYPE(FIELD_TYPE_ID) ON DELETE SET NULL
