...
CREATE TABLE ADD_FORM_TABLE (
    ...
    TAB_ID INT NOT NULL,                           -- connects to tab
    FIELD_SOURCE_LOV_ID INT NULL,                  -- e.g., dropdown source
    FIELD_NAME VARCHAR(100) NOT NULL,              -- actual name (e.g., emp_name)
    FIELD_SIZE_LOV_ID INT NULL,                    -- e.g., small, medium, large
    FIELD_ICON_LOV_ID INT NULL,                    -- e.g., user, email icon
    PLACEHOLDER VARCHAR(255) NULL,                 -- input hint text
    FIELD_ORDER_LOV_ID INT NULL,                   -- defines display order
    STATUS ENUM('active','inactive') DEFAULT 'active',
    INACTIVE_REASON VARCHAR(255) NULL,
    C2C_CDATE DATETIME DEFAULT CURRENT_TIMESTAMP,
    C2C_CUSER VARCHAR(100),
    ...
    FOREIGN KEY (FIELD_SOURCE_LOV_ID) REFERENCES LIST_OF_VALUES(LOV_ID) ON DELETE SET NULL,
    FOREIGN KEY (FIELD_SIZE_LOV_ID) REFERENCES LIST_OF_VALUES(LOV_ID) ON DELETE SET NULL,
    FOREIGN KEY (FIELD_ICON_LOV_ID) REFERENCES LIST_OF_VALUES(LOV_ID) ON DELETE SET NULL,
    FOREIGN KEY (FIELD_ORDER_LOV_ID) REFERENCES LIST_OF_VALUES(LOV_ID) ON DELETE SET NULL
);
